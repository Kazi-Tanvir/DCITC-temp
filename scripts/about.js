...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (fab) {
    fab.addEventListener("click", function () {
      const fabBgColor = getComputedStyle(rootElement)
        .getPropertyValue("--card-background")
        .includes("rgba")
        ? isDarkMode
          ? "#1a1a1a"
          : "#ffffff" // Use solid if card bg is transparent
        : getComputedStyle(rootElement).getPropertyValue("--card-background");
      const fabTextColor =
        getComputedStyle(rootElement).getPropertyValue("--text-primary");
      const fabPrimaryColor =
        getComputedStyle(rootElement).getPropertyValue("--primary-color");
      const fabSecondaryColor =
      ...
      Swal.fire({
        ...
        customClass: {
          // Ensure SweetAlert uses theme colors
  ...
  function showJoinForm() {
    ...
    Swal.fire({
      ...
      customClass: {
        ...
        cancelButton: "swal-themed-button-cancel",
      },
      preConfirm: () => {
