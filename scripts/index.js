...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (menuBtn && hamburger && navLinks && overlay && navbarContainer) {
        ...
        if (targetId.startsWith("#")) {
          ...
          if (targetElement) {
            ...
            const offsetPosition =
              elementPosition + window.pageYOffset - navbarHeight - 15; // 15px extra offset
    ...
    function highlightActiveLink() {
      ...
      if (
        ...
        const lastInternalLink = Array.from(navItems)
          .reverse()
          .find((link) => link.getAttribute("href").startsWith("#"));
        if (lastInternalLink) {
  ...
  if (txtElement) {
    const words = ["IT CLUB", "INNOVATION", "TECHNOLOGY"];
    const wait = 2000;
  ...
  const filterButtonsDept = document.querySelectorAll(
    ...
    button.addEventListener("click", () => {
      ...
      departmentCards.forEach((card) => {
        ...
        if (shouldShow) {
          ...
          setTimeout(() => {
            // Allow display change before animation
  ...
  function updateThemeColors() {
    ...
    if (sliderSection) {
      if (primaryColor)
        sliderSection.style.setProperty("--primary-color", primaryColor);
      if (secondaryColor)
        sliderSection.style.setProperty("--secondary-color", secondaryColor);
      if (primaryRgb)
        sliderSection.style.setProperty("--primary-rgb", primaryRgb);
      if (primaryColor)
        sliderSection.style.setProperty("--button-background", primaryColor);
      if (secondaryColor)
        sliderSection.style.setProperty("--button-hover", secondaryColor);
    }
  ...
  function navigateToSlide(index) {
    ...
    const currentSlideContent =
      slides[currentSlide].querySelector(".slide-content");
  ...
  function initTiltEffect() {
    if (window.innerWidth > 992 && typeof $ !== "undefined" && $.fn.tilt) {
      $(".slide-image:not(.tilt-initialized)")
        .tilt({
          maxTilt: 5,
          perspective: 1000,
          scale: 1.05,
      ...
      $(".slide-image.tilt-initialized")
        .tilt()
        .tilt.destroy.call($(".slide-image.tilt-initialized"))
        .removeClass("tilt-initialized");
    }
  }
  ...
  function handleResize() {
    ...
    if (isMobile) {
      ...
      if (sliderSection && currentSlideElement) {
        const slideContent =
          currentSlideElement.querySelector(".slide-content");
        if (slideContent) {
  ...
  const allEventCards = eventCardsContainer
    ? Array.from(eventCardsContainer.querySelectorAll(".event-card"))
    : [];
  ...
  function filterAndDisplayEvents() {
    ...
    allEventCards.forEach((card, index) => {
      ...
      const matchesFilter =
        ...
        category === currentEventFilter.replace(/s$/, ""); // Handle plural 's'
    ...
    if (emptyState)
      emptyState.style.display = visibleEventCount === 0 ? "block" : "none";
    if (loadMoreBtn) {
      const totalMatching = allEventCards.filter((card) => {
        const cat = card.getAttribute("data-category")?.toLowerCase() || "";
        return (
          currentEventFilter === "all events" ||
          cat === currentEventFilter.replace(/s$/, "")
        );
      }).length;
      loadMoreBtn.style.display =
        visibleEventCount < totalMatching ? "block" : "none";
  ...
  if (loadMoreBtn) {
    loadMoreBtn.addEventListener("click", function () {
      ...
      let newlyShown = 0;
      let currentVisible = allEventCards.filter(
      ...
      allEventCards.forEach((card) => {
        if (card.style.display === "none" && newlyShown < eventsPerPage) {
          ...
          const matchesFilter =
            currentEventFilter === "all events" ||
            category === currentEventFilter.replace(/s$/, "");
          if (matchesFilter) {
            card.style.display = "flex";
            if (typeof AOS !== "undefined") {
              // Apply AOS to newly loaded items
      ...
      setTimeout(() => {
        ...
        const nowVisible = allEventCards.filter(
          (card) => card.style.display === "flex"
        ).length;
  ...
  function validateInput(input) {
    ...
    if (input.required && value === "") {
      isValid = false;
      errorMessage = `${
        input.labels[0]?.textContent || "This field"
      } is required`;
