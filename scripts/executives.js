document.addEventListener("DOMContentLoaded", function () {
  ...
  if (
    typeof particlesJS !== "undefined" &&
    document.getElementById("footer-particles-js")
  ) {
    ...
    console.warn(
      "Particles.js library or #footer-particles-js element not found."
    );
  }
  ...
  function filterTeamMembers() {
    ...
    const activeDepartment =
      document.querySelector(".department-btn.active")?.dataset.department ||
      "all";
    const departmentFilterContainer = document.querySelector(
    ...
    yearContentSections.forEach((section) => {
      section.style.display =
        section.dataset.year === activeYear ? "block" : "none";
    });
    if (facultySection) facultySection.style.display = "block";

    // 2. Show/Hide Department Filters
    if (departmentFilterContainer) {
      departmentFilterContainer.style.display =
        activeYear === "2023-2024" ? "flex" : "none";
    }
    ...
    teamCards.forEach((card, index) => {
      ...
      if (isFaculty || cardYear === activeYear) {
        const memberData = card.dataset.memberJson
          ? JSON.parse(card.dataset.memberJson)
          : {};
        ...
        if (
          ...
          // Support exact department match, including Public Relations
          matchesDepartment = memberData.department === activeDepartment;

          // Special case handling for department names with special characters if needed
          if (!matchesDepartment && activeDepartment === "Public Relations") {
            matchesDepartment =
              department.includes("public") && department.includes("relation");
          }
        }
    ...
    if (noResultsMessage) {
      const activeYearSectionIsVisible =
        document.querySelector(`.year-content[data-year="${activeYear}"]`)
          ?.style.display === "block";
      noResultsMessage.style.display =
        visibleCount === 0 && activeYearSectionIsVisible ? "block" : "none";
    }
  }
  ...
  yearButtons.forEach((button) => {
    button.addEventListener("click", function () {
      ...
      document
        .querySelector('.department-btn[data-department="all"]')
        ?.classList.add("active");
      filterTeamMembers();
      if (typeof toastr !== "undefined")
        toastr.info(`Showing team for ${this.dataset.year}`);
    });
  });
    ...
    card.addEventListener("click", function () {
      ...
      try {
        ...
        modal.querySelector("#modalName").textContent =
          memberData.name || "N/A";
        modal.querySelector("#modalPosition").textContent =
          memberData.position || "N/A";
        modal.querySelector("#modalDepartment").textContent =
          memberData.department || "N/A";
        modal.querySelector("#modalBio").textContent =
          memberData.bio || "No biography available.";
        const imgSrc =
          this.querySelector("img")?.src ||
          "https://placehold.co/300x400/00b4db/ffffff";
        modal.querySelector("#modalImage").src = imgSrc;
        modal.querySelector("#modalImage").alt =
          ...
          if (memberData.social && typeof memberData.social === "object") {
            Object.entries(memberData.social).forEach(([platform, url]) => {
              if (url && url !== "#") {
                ...
                else if (platform.includes("codepen"))
                  iconClass = "fab fa-codepen";
                else if (platform.includes("hackthebox"))
                  iconClass = "fas fa-box";
                else if (platform.includes("security"))
                ...
                else if (platform.includes("kaggle"))
                  iconClass = "fab fa-kaggle";
                else if (platform.includes("medium"))
                  iconClass = "fab fa-medium";

                const link = document.createElement("a");
                link.href = url.startsWith("mailto:")
                  ? url
                  : url.startsWith("http")
                  ? url
                  : `//${url}`; // Basic URL prepending
                link.target = "_blank";
        ...
        if (typeof toastr !== "undefined")
          toastr.error("Could not load member details.");
      }
    });
  });
  ...
  function navigateTeamMember(direction) {
    if (currentCardIndex === -1 || !modal || !modal.classList.contains("show"))
      return;

    // Get only visible cards that match current filters
    const visibleCards = Array.from(teamCards).filter(
    ...
    const currentVisibleIndex = visibleCards.findIndex(
      (card) => allCards.indexOf(card) === currentCardIndex
    );
    ...
    try {
      ...
      setTimeout(() => {
        ...
        modal.querySelector("#modalBio").textContent =
          memberData.bio || "No biography available.";
        const imgSrc =
          newCard.querySelector("img")?.src || "/api/placeholder/300/400";
        modal.querySelector("#modalImage").src = imgSrc;
        modal.querySelector("#modalImage").alt =
          memberData.name || "Team Member";
      ...
      if (typeof toastr !== "undefined")
        toastr.error("Could not navigate to team member.");
    }
  }
  ...
  // Initialize filter on page load (run twice to ensure all cards are properly processed)
  setTimeout(() => {
    // Force re-initialization of team cards array
    const teamCards = document.querySelectorAll(".team-card");
    const allCards = Array.from(teamCards);

    // Fix any JSON parsing issues in data attributes
    teamCards.forEach((card) => {
      try {
        // Validate JSON format
        if (card.dataset.memberJson) {
          JSON.parse(card.dataset.memberJson);
        }
      } catch (e) {
        console.error("Invalid JSON in card:", card);
        // Try to repair common JSON issues
        const fixedJson = card.dataset.memberJson
          .replace(/(['"])?([a-zA-Z0-9_]+)(['"])?:/g, '"$2":') // Fix unquoted keys
          .replace(/'/g, '"'); // Replace single quotes with double quotes
        card.dataset.memberJson = fixedJson;
      }
    });

    // Force initialize filters
    filterTeamMembers();

    // Re-initialize tilt effect
    if (typeof $.fn.tilt === "function") {
      $(".team-card:visible").tilt({
        glare: true,
        maxGlare: 0.1,
        maxTilt: 5,
        scale: 1.02,
